...
| bs   | cell            | local  | stub  | total  | vs KVM (local/stub 增量占比)          |
| :--- | :-------------- | :----- | :---- | :----- | :-------------------------------- |
| 64K  | SVM zc=0        | 881357 | 34326 | 915721 | +7.3% (local +3.3% / stub +4.0%)  |
| 64K  | SVM zc=4M(copy) | 907998 | 34029 | 942051 | +10.4% (local +6.4% / stub +4.0%) |
| 64K  | KVM             | 853256 | —     | 853256 | —                                 |
| 512K | SVM zc=0        | 899083 | 35745 | 935011 | +6.7% (local +2.7% / stub +4.1%)  |
| 512K | SVM zc=4M(copy) | 918365 | 36009 | 954489 | +9.0% (local +4.9% / stub +4.1%)  |
| 512K | KVM             | 875783 | —     | 875783 | —                                 |
| 1M   | SVM zc=0        | 885613 | 37530 | 923415 | +5.1% (local +0.8% / stub +4.3%)  |
| 1M   | SVM zc=4M(copy) | 907761 | 38969 | 946998 | +7.8% (local +3.4% / stub +4.4%)  |
| 1M   | KVM             | 878215 | —     | 878215 | —                                 |
| 2M   | SVM zc=0        | 913590 | 47139 | 961156 | +8.9% (local +3.5% / stub +5.3%)  |
| 2M   | SVM zc=4M(copy) | 905434 | 48166 | 954106 | +8.1% (local +2.6% / stub +5.5%)  |
| 2M   | KVM             | 882431 | —     | 882431 | —                                 |

（stub 恒定 34–48MB，skmem KB 级；local RSS 跨 cell 基线波动 ±5%，% 仅作量级参考，同 bs 内 zc/copy 背靠背可比。括号内占比均以 KVM 同 bs 的 total 为基准，local% + stub% ≈ 总增量%。）
...
4. **远程路径的每字节成本（KVM 是必要的分母）**
   - 单请求延迟随 bs 涨 16K→2M：KVM 只涨 4.5×（19.8→88.5µs），SVM P=off 涨 16.8×（48.7→818µs）
   - 这 16.8× 里，**盘本身只该贡献 4.5×**（KVM 同 bs 扫出来的就是盘+本地 virtio 的固有代价），剩下 12.3× 的放大归因于 local→TCP→stub 的拷贝/传输/aio 每字节成本
   - **没有 KVM 这组，16.8× 无法归因**——你没法分辨延迟暴涨是盘相位/bs 特性，还是远程转发路径的问题
