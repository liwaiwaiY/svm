#!/bin/bash
# run_macro_kvm_anchor.sh - KVM TPCC back-to-back anchor (same disk state as SVM v2).
#   threads {1,2,4} x 1 run each, 60s, O_DIRECT + drop_caches. Output -> exp/results/macrokvm/anchor/
set -u
H=/home/waiai/svm
OUT=$H/exp/results/macrokvm/anchor
mkdir -p $OUT/logs
...
pkill -f "qemu-system-x86_64" 2>/dev/null; pkill -f "remote-stub" 2>/dev/null; sleep 3
...
for t in 1 2 4; do
  ...
  env $SSH "cd /home/wai/sysbench-tpcc && sysbench --db-driver=mysql --mysql-host=127.0.0.1 --mysql-port=3306 --mysql-user=sbtest --mysql-password=sbtest --mysql-db=tpcc --tables=1 --scale=1 --time=60 --threads=$t ./tpcc.lua run" 2>/dev/null \
    > $OUT/logs/kvm-anchor-t$t.log
  python3 -c "
import re
s=open('$OUT/logs/kvm-anchor-t$t.log').read()
...
print(f'  KVM-anchor t$t: txn={txn.group(1)} ({txn.group(2)}/s) avg={lat.group(1)}ms p95={p95.group(1)}ms max={mx.group(1)}ms')
" 2>/dev/null || echo "  KVM-anchor t$t: PARSE FAIL"
done
env $SSH "echo wai | sudo -S bash -c 'systemctl stop mysql; sleep 2; poweroff'" 2>/dev/null
sleep 12
pkill -f "qemu-system-x86_64" 2>/dev/null
echo "KVM_ANCHOR_OK [$(date +%T)]"
