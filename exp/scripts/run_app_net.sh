#!/bin/bash
# run_app_net.sh <nvmeof|iscsi>
# App (Spark pagerank) on network-storage backends; guest data disk vdb = host initiator device.
...
#   SPARK_HOME + wiki + spark.local.dir all on /mnt/data (data disk, cache=none).
#   cores {1,2,4} x 1 run, 10 iterations, drop_caches per run, graceful shutdown.
#   Output -> exp/results/app-net/$PROTO/
set -u
PROTO=$1
H=/home/waiai/svm
OUT=$H/exp/results/app-net/$PROTO
mkdir -p $OUT/logs
...
echo "=== [$(date +%T)] APP-NET $PROTO (dev=$DEV): start ==="
...
setsid $H/local_qemu/build/qemu-system-x86_64 \
  ...
  -serial file:/tmp/app-net-guest.log -monitor none -display none \
  -qmp unix:/tmp/qmp.sock,server,nowait \
  > /tmp/app-net-out.log 2>&1 < /dev/null &
wait_ssh
mount_data
...
for c in "${CS[@]}"; do
  ...
  env $SSH "export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64; export SPARK_HOME=/mnt/data/spark; export SPARK_LOCAL_DIRS=/mnt/data/spark-tmp; cd /home/wai && timeout 3600 \$SPARK_HOME/bin/spark-submit --master local[$c] /home/wai/pagerank.py /mnt/data/wiki/wiki-Talk-1m.txt 10 $c" 2>/dev/null \
    > $OUT/logs/$PROTO-c$c-r1.log
  parse $OUT/logs/$PROTO-c$c-r1.log "$PROTO c$c r1"
done
...
echo "APP_NET_${PROTO}_OK [$(date +%T)]"
